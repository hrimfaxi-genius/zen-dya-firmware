...
struct iqs7211e_data {
    ...
    bool tap_suppressed_until_release;
    uint16_t x_resolution;
    uint16_t y_resolution;
    bool resolution_valid;
    bool gesture_started_near_edge;
    uint8_t scroller_axis_lock;
    int32_t two_finger_wheel_remainder;
    int32_t two_finger_hwheel_remainder;
...
static int16_t iqs7211e_scale_two_finger_scroll_delta(int32_t *remainder, int16_t delta) {
#if CONFIG_IQS7211E_TWO_FINGER_SCROLL_DIVIDER <= 1
    ARG_UNUSED(remainder);
    return delta;
#else
    int32_t scaled = *remainder + delta;
    int32_t out = scaled / CONFIG_IQS7211E_TWO_FINGER_SCROLL_DIVIDER;

    *remainder = scaled % CONFIG_IQS7211E_TWO_FINGER_SCROLL_DIVIDER;
...
static void iqs7211e_report_scroll(struct iqs7211e_data *data, uint16_t axis,
    ...
    data->tap_suppressed_until_release = true;
...
static void iqs7211e_motion_work_handler(struct k_work *work) {
    ...
    if (finger_count == 1) {
        ...
        if (!data->finger_2_prev_valid) {
            ...
            data->last_touch_time = current_time;
            data->scroll_was_active = false;
            data->tap_suppressed_until_release = false;
            data->two_finger_wheel_remainder = 0;
            data->two_finger_hwheel_remainder = 0;
            data->gesture_started_near_edge = iqs7211e_is_near_edge(data, finger_1_x, finger_1_y) ||
            ...
            int32_t movement_distance = abs(x_movement) + abs(y_movement);

            if (abs(x_movement) > CONFIG_IQS7211E_TWO_FINGER_SCROLL_JUMP_THRESHOLD ||
                abs(y_movement) > CONFIG_IQS7211E_TWO_FINGER_SCROLL_JUMP_THRESHOLD) {
                LOG_DBG("Ignoring two-finger jump: x=%d y=%d", x_movement, y_movement);
                data->tap_suppressed_until_release = true;
                data->two_finger_wheel_remainder = 0;
                data->two_finger_hwheel_remainder = 0;
                data->previous_x = finger_1_x;
                ...
                goto two_finger_done;
            }

            if (movement_distance >= CONFIG_IQS7211E_TAP_SUPPRESS_DISTANCE) {
                data->tap_suppressed_until_release = true;
            }
            ...
            if (cfg->scroller_mode) {
                ...
                int16_t v_delta = iqs7211e_scale_two_finger_scroll_delta(
                    &data->two_finger_wheel_remainder, -y_movement);
                int16_t h_delta = iqs7211e_scale_two_finger_scroll_delta(
                    &data->two_finger_hwheel_remainder, x_movement);

                if (v_delta != 0) {
                    LOG_DBG("Scroll Y: %d (raw %d)", v_delta, -y_movement);
                    input_report_rel(dev, INPUT_REL_WHEEL, v_delta, true, K_FOREVER);
                    data->scroll_was_active = true;
                    data->tap_suppressed_until_release = true;
                }
                if (h_delta != 0) {
                    LOG_DBG("Scroll X: %d (raw %d)", h_delta, x_movement);
                    input_report_rel(dev, INPUT_REL_HWHEEL, h_delta, true, K_FOREVER);
                    data->scroll_was_active = true;
                    data->tap_suppressed_until_release = true;
                }
            }
...
two_finger_done:
        ...
        if (data->previous_valid || data->finger_2_prev_valid) {
            ...
            tap_allowed = !(data->gesture_started_near_edge || ended_near_edge ||
                            data->tap_suppressed_until_release);

            if (data->finger_2_prev_valid) {
                // Two finger tap - right click
                if (tap_allowed && touch_duration < 200) { // Quick tap
                ...
                if (tap_allowed && touch_duration < 200 && tap_distance < 50) { // Quick tap with minimal movement
                    ...
                    LOG_DBG("Single finger tap ignored");
                    data->tap_count = 0;
                    data->double_tap_hold = false;
                }
